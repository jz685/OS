...
//	Declare imagetype to be a typedef'ed name of an anonymous enumeration ?
typedef enum {
	JPG, 
	PNG,
	BMP,
		...
		else 					// it is a file
		{
			...
			if (!strcmp(args->mode, "v1"))
			{
				...
				if ((typecheck(d_name) == JPG && id == 1) || (typecheck(d_name) == PNG && id == 2) || (typecheck(d_name) == BMP && id == 3) || (typecheck(d_name) == GIF && id == 4))
				{
					...
					//printf("The file is not JPG, PNG, BMP, GIF file, skipped it.\n");
				}
			}
